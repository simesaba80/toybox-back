...
func TestCommentUsecase_GetCommentsByWorkID(t *testing.T) {
	tests := []struct {
		name      string
		workID    uuid.UUID
		setupMock func(*usecase.MockCommentRepository, uuid.UUID)
		wantCount int
		wantErr   bool
	}{
		{
			name:   "正常系: コメント取得成功",
			workID: uuid.New(),
			setupMock: func(m *usecase.MockCommentRepository, workID uuid.UUID) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockRepo := usecase.NewMockCommentRepository(ctrl)
			tt.setupMock(mockRepo, tt.workID)
			uc := usecase.NewCommentUsecase(mockRepo, 30*time.Second)
